...
import com.datacruncher.eventtrigger.DynamicClassLoader;
import com.datacruncher.eventtrigger.EventTrigger;
		...
		if(StringUtils.isEmpty(code) || StringUtils.isEmpty(name)){
			...
			try{
					...
					if (clazz != null) {
						...
						if(ArrayUtils.isNotEmpty(interfaces)){
							for (Class clz : interfaces) {
								if(clz.getName().equalsIgnoreCase("EventTrigger")){
									isValid = true;
								}
							}
						} else if(clazz.getSuperclass() != null && clazz.getSuperclass().getName().equalsIgnoreCase("EventTriggerImpl")) {
							isValid = true;
						}
					}
				}
				if(isValid){
					...
					result = I18n.getMessage("error.trigger.wrongimpl");//"Failed. Reason: Custom code should implement EventTrigger Interface";
				}
			}catch(Exception e){
