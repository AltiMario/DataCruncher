/*
 * Copyright (c) 2015  www.see-r.com
 * All rights reserved
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

package com.seer.datacruncher.datastreams;

import com.seer.datacruncher.constants.ValidationStep;

public class DatastreamDTO {
	private long idSchema;
	private String input;
	private String output;
	private boolean success;
	private String message;
	private int idStreamType;
	private boolean isWarning;
    private Object jaxbObject;
    private Object jvObject;
    private ValidationStep errorLevel;
	
	public void setIdSchema(long idSchema) {
		this.idSchema = idSchema;
	}

    public Object getJvObject() {
        return jvObject;
    }

    public void setJvObject(Object jvObject) {
        this.jvObject = jvObject;
    }

    public long getIdSchema() {
		return idSchema;
	}
	
	public String getInput() {
		return input;
	}
	
	public void setInput(String input) {
		this.input = input;
	}
	
	public String getOutput() {
		return output;
	}
	
	public void setOutput(String output) {
		this.output = output;
	}
	
	public boolean getSuccess() {
		return success;
	}
	
	public void setSuccess(boolean success) {
		this.success = success;
	}
	
	public String getMessage() {
		return message;
	}
	
	public void setMessage(String message) {
		this.message = message;
	}

	public int getIdStreamType() {
		return idStreamType;
	}

	public void setIdStreamType(int idStreamType) {
		this.idStreamType = idStreamType;
	}

	public boolean isWarning() {
		return isWarning;
	}

	public void setWarning(boolean isWarning) {
		this.isWarning = isWarning;
	}

    public Object getJaxbObject() {
        return jaxbObject;
    }

    public void setJaxbObject(Object jaxbObject) {
        this.jaxbObject = jaxbObject;
    }

	public ValidationStep getErrorLevel() {
		return errorLevel;
	}

	public void setErrorLevel(ValidationStep errorLevel) {
		this.errorLevel = errorLevel;
	}
}
