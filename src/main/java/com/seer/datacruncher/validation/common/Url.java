...
public class Url implements SingleValidation, Serializable {
     ...
     *  a {@link Regex} instead ({@link #Url(Regex, long)})  
     */
     ...
     */
    private final Regex authorityValidator;
     ...
     */
    private static final Url DEFAULT_URL_VALIDATOR = new Url();
     ...
     * ALLOW_2_SLASHES + NO_FRAGMENTS enables both of those options.
     */
    public Url(String[] schemes, long options) {
     ...
     */
    public Url(Regex authorityValidator, long options) {
     ...
     * enables both of those options.
     */
    public Url(String[] schemes, Regex authorityValidator, long options) {
     ...
     */
    public ResultStepValidation checkValidity(String url) {
        ...
        try{
            if(isValid(url)){
                ...
                result.setMessageResult("URL: [" + url + "] wrong.\n"+invalidCause );
            }
        }catch(Exception e)  {
            result.setValid(false);
            result.setMessageResult("URL: [" + url + "] wrong. " +e.getMessage());
        }
        return result;
    }
    /**
    ...
    public boolean isValid(String value) {
        if (value == null) {
            invalidCause = "Url is NULL.";
            return false;
        }

        if (!ASCII_PATTERN.matcher(value).matches()) {
            invalidCause = "Url does not match to ASCII Pattern.";
            return false;
        }
        ...
        if (!urlMatcher.matches()) {
            invalidCause = "Url does not match to URL Pattern.";
            return false;
        }

        String scheme = urlMatcher.group(PARSE_URL_SCHEME);
        if (!isValidScheme(scheme)) {
            invalidCause = "Url does not match to SCHEME Pattern.";
            return false;
        }
        ...
        if ("file".equals(scheme) && "".equals(authority)) {
            ...
            if (!isValidAuthority(authority)) {
                invalidCause = "Authority does not valid";
                return false;
            }
        }

        if (!isValidPath(urlMatcher.group(PARSE_URL_PATH))) {
            invalidCause = "Url does not match to PATH Pattern.";
            return false;
        }

        if (!isValidQuery(urlMatcher.group(PARSE_URL_QUERY))) {
            invalidCause = "Url does not match to QUERY Pattern.";
            return false;
        }

        if (!isValidFragment(urlMatcher.group(PARSE_URL_FRAGMENT))) {
            invalidCause = "Url does not match to FRAGMENT Pattern.";
            return false;
        }
    ...
    protected boolean isValidAuthority(String authority) {
        ...
        Domain domainValidator = Domain.getInstance(isOn(ALLOW_LOCAL_URLS));
