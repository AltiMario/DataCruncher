...
public class ConnectionsDestroyController implements Controller, DaoSet {
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if(connectionId != null && connectionId.startsWith("/"))
			connectionId = connectionId.substring(1);
		
		boolean isAnyJobDeactivated = SchedulerUtils.stopJobsByConnectedField(plannerJobMap,
				SchedulerUtils.FIELD_CONNECTION, Long.parseLong(connectionId));
		Destroy destroy = connectionsDao.destroy(Long.parseLong(connectionId));
