...
import com.seer.datacruncher.services.scheduler.PlannerJob;
...
public class JobsSetIsActiveController implements Controller, DaoSet {
	
    private static Logger log = Logger.getLogger(JobsSetIsActiveController.class);	
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		 ...
		 * Validators. 
		 * 
		 * Note: Applcation and schema mapping checker is in JobsDao (onUpdate).
		 */
		if(jobEnt.getIdEventTrigger() > 0) {
			if (jobEnt.getIdScheduler() == 0) {
				// this validation is actually fired on client-side (jobsContent.js)
		...
		if(isSuccess) {
			...
			//CronUtils.getCronMask((new TasksDao()).find(jobEnt.getIdScheduler()));
			if (isActive == 1) {
				try {
					plannerJob.schedule();
	...
	/**
	 * Checks whether planner's entity is complete.
	 * 
	 * @param ent - Planner's entity
	 * @return
	 */
	...
	private boolean isConnectionSuccess(ConnectionsEntity ent) {
		boolean success;
	    success = checkServiceRunning(ent);
		return success;
	}
