...
Ext.extend(Ext.App, Ext.util.Observable, {
    ...
    addMessage : function(status, msg) {
        var delay = 3000;    // <-- default delay of msg box is 1 second.
        if (status == false) {
            delay = 5000;    // <-- when status is error, msg box delay is 3 seconds.
        }
		...
		Ext.core.DomHelper.append(this.msgCt, {html:this.buildMessageBox(status, Array.prototype.slice.call(arguments, 1))}, true).slideIn('t').pause(delay).ghost("t", {remove:true});
    },
